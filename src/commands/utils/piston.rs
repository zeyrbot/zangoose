...
#[poise::command(prefix_command, slash_command, rename = "piston", aliases("run", "code"))]
pub async fn run(
    ctx: ZeyrContext<'_>,
        ...
        Ok(response) => {
            ...
            if !response.run.stderr.is_empty() {
                final_content.push_str(
                    &format!("error: {}", response.run.stderr)
                );
            } else {
                final_content.push_str(
                    &format!("output: {}", CodeBlock { code: response.run.output, language: Some(response.language.to_lowercase()) })
                );
            }
